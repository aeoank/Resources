...
#define Finger_RST_Pin 8
#define Finger_WAKE_Pin 9
...
// For Arduino, use SoftwareSerial for the fingerprint module
#include <SoftwareSerial.h>
#define FP_RX 10  // Connect to TX of fingerprint module
#define FP_TX 11  // Connect to RX of fingerprint module

SoftwareSerial FingerSerial(FP_RX, FP_TX);

void setup() {
  Serial.begin(115200);
  FingerSerial.begin(19200); // Default baud rate for many modules
...
uint8_t TxAndRxCmd(uint8_t Scnt, uint8_t Rcnt, uint16_t Nms){
  ...
  do {
    ...
    if(FingerSerial.available()) {
      finger_RxBuf[uart_RxCount++] = FingerSerial.read();
    }
    time_after = millis(); 
...
void Finger_Wait_Until_OK(void){
  ...
  Serial.print("Number of fingerprints already available: "); Serial.println(GetUserCount());
  Serial.println(" Use the serial port to send the commands to operate the module:");
  Serial.println(" CMD1 : Query the number of existing fingerprints"); 
  Serial.println(" CMD2 : Add fingerprint (Each entry needs to be read two times: \"beep\", put the finger on sensor)"); 
  Serial.println(" CMD3 : Fingerprint matching (Send the command, then put your finger on sensor. Each time you send a command, module waits and matches once)"); 
  Serial.println(" CMD4 : Clear fingerprints"); 
  Serial.println(" CMD5 : Switch to sleep mode, you can use the finger Automatic wake-up function"); 
  Serial.println(" CMD6 : Wake up and make all commands valid");
  Serial.println("*************** WaveShare Capacitive Fingerprint Reader Test ***************");
}
    ...
    switch(step) {
      ...
      case 3:  
        switch(temp) {      
          ...
          case '2':
            if(Finger_SleepFlag == 1) break;
            Serial.println(" Add fingerprint (Each entry needs to be read two times: put the finger on sensor)"); 
            switch(AddUser()) {
              case ACK_SUCCESS:
...
void Auto_Verify_Finger(void){
  if(digitalRead(Finger_WAKE_Pin)!=LOW){  
    ...
    delay(1000); 
    FingerSerial.begin(19200);
    Serial.println("Please keep putting your finger on it");
    switch(VerifyUser()) {       
      case ACK_SUCCESS: 
